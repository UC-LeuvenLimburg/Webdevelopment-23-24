...
function getLocationsData(campussen) {
    var campussenWithLocation = [];
    return new Promise((resolve, reject) => {
        for (var campus of campussen) {
            getLocationData(campus)
                .then(response => {
                    campussenWithLocation.push(response);
                    if (campussenWithLocation.length == campussen.length) {
                        resolve(campussenWithLocation);
                    }
                })
                .catch(error => { reject(new Error("getLocationsData error:" + error)); })
        }
    })
}
